...
export default function ClientPage() {
  ...
  function Service({
    ...
    if (inService) {
      return (
        <button key={keyID} onClick={() => navigate(keyID)}>
          {serviceType}
        </button>
      );
    } else {
      return (
        <button key={keyID} onClick={() => changeService(serviceType)}>
          {serviceType}
        </button>
      );
    }
        ...
        <div className="serviceContainer">
          ...
          <div id="servicesOptions">
            ...
            {selectedService.map((service, i) => (
              <Service serviceType={service} keyID={i + 1} key={i} />
            ))}
          </div>
        </div>
            ...
            {listOfUpcomingAppts.map((apptId, count) => {
              ...
              return (
                <div key={apptId} className="apptIndiv">
                  ...
                  <p className="deets">
                    Freelancers: {details.Freelancers.join(", ")}
                  </p>
                </div>
              );
            })}
